...
public class FreezeTopFenLayerFeature extends Feature<DefaultFeatureConfig> {
    public FreezeTopFenLayerFeature(Codec<DefaultFeatureConfig> codec) {
        super(codec);
    }
        ...
        for (int i = 0; i < 16; ++i) {
            for (int j = 0; j < 16; ++j) {
                ...
                if (biome.canSetIce(structureWorldAccess, mutable2, false)) {
                    structureWorldAccess.setBlockState(mutable2, PermafrozenBlocks.FEN_ICE.getDefaultState(), Block.NOTIFY_LISTENERS);
                }
